...
const Sidebar = ({ mobileOpen = false, onClose = () => {} }) => {
  ...
  if (!user || !user.role || !internalRoles.includes(user.role) || user.role === 'Client') {
      ...
      return (
        <>
          ...
          <aside className={`w-64 bg-slate-900 border-r border-slate-800 h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl text-white font-sans transition-transform duration-300 ease-in-out antialiased ${
            mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
              ...
              <div className="p-5 border-b border-slate-800 flex items-center justify-between">
                  ...
                  <button 
                    onClick={onClose}
                    className="p-1 text-slate-400 hover:text-white rounded-lg lg:hidden cursor-pointer border-none bg-transparent"
                  >
                    <X size={18} />
                  </button>
              </div>
  ...
  if (isSales) {
    menuItems = [
      { label: 'Sales Dashboard', path: '/app/dashboard', icon: <LayoutDashboard size={20} /> },
      { label: 'CRM Contacts', path: '/app/contacts', icon: <Users size={20} /> },
      { label: 'Sales Pipeline', path: '/app/sales', icon: <Files size={20} />, hasBadge: true },
      { label: 'Products & Catalog', path: '/app/products', icon: <Package size={20} /> },
      { label: 'Sales Tasks', path: '/app/tasks', icon: <ListTodo size={20} /> },
      { label: 'Meetings Calendar', path: '/app/calendar', icon: <Calendar size={20} /> },
      { label: 'Client Inquiries', path: '/app/tickets', icon: <Ticket size={20} /> },
    ];
  } else if (isHR) {
    menuItems = [
      { label: 'HR Dashboard', path: '/app/dashboard', icon: <LayoutDashboard size={20} /> },
      { label: 'Staff Directory', path: '/app/users', icon: <UserPlus size={20} /> },
      { label: 'Workforce Attendance', path: '/app/attendance', icon: <Clock size={20} /> },
      { label: 'Payroll Processing', path: '/app/payroll', icon: <DollarSign size={20} /> },
      { label: 'HR Tasks', path: '/app/tasks', icon: <ListTodo size={20} /> },
      { label: 'Company Calendar', path: '/app/calendar', icon: <Calendar size={20} /> },
    ];
  } else {
    // Regular Employee / Technical Staff
    menuItems = [
      { label: 'My Dashboard', path: '/app/dashboard', icon: <LayoutDashboard size={20} /> },
      { label: 'My Assigned Tasks', path: '/app/tasks', icon: <ListTodo size={20} /> },
      { label: 'Schedule Calendar', path: '/app/calendar', icon: <Calendar size={20} /> },
      { label: 'Daily Clock-in', path: '/app/attendance', icon: <Clock size={20} /> },
      { label: 'My Payslips', path: '/app/payroll', icon: <DollarSign size={20} /> },
      { label: 'Support Tickets', path: '/app/tickets', icon: <Ticket size={20} /> }
    ];
  }
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-white border-r border-slate-200/80 h-screen fixed left-0 top-0 flex flex-col z-50 shadow-2xl lg:shadow-xs font-sans transition-transform duration-300 ease-in-out antialiased ${
        mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
            ...
            <button 
              onClick={onClose}
              className="p-1 text-slate-400 hover:text-slate-700 rounded-lg lg:hidden cursor-pointer border-none bg-transparent"
            >
              <X size={18} />
            </button>
        </div>
        ...
        <div className="p-3 border-t border-slate-100 space-y-1 shrink-0">
          <NavLink 
              to="/app/settings"
              className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-xs w-full ${
          ...
          <button 
              type="button"
              className="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-xs w-full text-rose-600 hover:bg-rose-50 cursor-pointer border-none bg-transparent" 
              onClick={logout}
          >
              <LogOut size={16} />
              Sign Out
